...
int main(int argc, char** argv)
{
	...
	traj_time_flat_x,traj_time_y,traj_time_acc_y,traj_time_dec_y,traj_time_flat_y,Table,dt);
    cout << "accTabel:" << endl;
	...
	while(fabs(x1)>v_max*dt | fabs(y1)>v_max*dt | fabs(v0_x)>a_max*dt | fabs(v0_y)>a_max*dt){
		...
		if(debug_flag){
			...
			out_txt_file<< setw(10)<< setprecision(1)<<CaseX;
			out_txt_file<< setw(10)<< setprecision(1)<<CaseY;
			out_txt_file<< '\n';
			// cout<< "ax:"<<a[0]<<endl;
			// cout<< "ay:"<<a[1]<<endl;
			// cout<< "vx:"<<v[0]<<endl;
